...
dataname <- paste0(CD, "_DLNM_morbCir_modeldata.RData")
loaddata <- file.exists(dataname)

modelname <- file.path(paste0(CD, "-", maxL, "-Cir-modeldata.RData"))
...
pw1 <- pw1 + #geom_line(aes(x = l, y = ul), linetype = "dashed", linewidth = 1.5, alpha = 0.7, color = "#2c7bb6") + 
    #geom_line(aes(x = l, y = ll), linetype = "dashed", linewidth = 1.5, alpha = 0.7, color = "#2c7bb6")
    geom_ribbon(aes(x = l, ymax = ul, ymin = ll), alpha = 0.3, fill = "#2c7bb6")
...
pw1 <- pw1 + labs(# title = paste0(CDname, ": Circulatory"),
                  subtitle = "ACE-DLNM: w")
pw_dlnm <- pw1
...
p1 <- annotate_figure(p1, top = text_grob(paste0(CDname, ": Circulatory"), 
               color = "black", face = "bold", size = 1.2*GGPLOTSTSIZE))
ggsave(p1, file = file.path("figures", paste0(CD, "-Cir-w-f.pdf")), width = 3*PLOTWIDTH, height = PLOTHEIGHT)

ptemp <- ggarrange(ptemp_dlnm, ptemp_mgcv, nrow=1, common.legend = TRUE, legend="right")
ggsave(ptemp, file = file.path("figures", paste0(CD, "-Cir-temp.pdf")), width = 2*PLOTWIDTH, height = PLOTHEIGHT)

ptime <- ggarrange(ptime_dlnm, ptime_mgcv, nrow=1, common.legend = TRUE, legend="right")
ggsave(ptime, file = file.path("figures", paste0(CD, "-Cir-time.pdf")), width = 2*PLOTWIDTH, height = PLOTHEIGHT)

pmonth <- ggarrange(pmonth_dlnm, pmonth_mgcv, nrow=1, common.legend = TRUE, legend="right")
ggsave(pmonth, file = file.path("figures", paste0(CD, "-Cir-month.pdf")), width = 2*PLOTWIDTH, height = PLOTHEIGHT)

p_qq <- ggarrange(qq_dlnm, qq_mgcv, nrow=1, common.legend = TRUE, legend="right")
ggsave(p_qq, file = file.path("figures", paste0(CD, "-Cir-qq.pdf")), width = 2*PLOTWIDTH, height = PLOTHEIGHT)

p_res <- ggarrange(res_dlnm, res_mgcv, nrow=1, common.legend = TRUE, legend="right")
ggsave(p_res, file = file.path("figures", paste0(CD, "-Cir-res.pdf")), width = 2*PLOTWIDTH, height = PLOTHEIGHT)
